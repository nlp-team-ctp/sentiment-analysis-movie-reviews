...
class LldaModel:
    ...
    def _load_labeled_documents(self, labeled_documents):
        ...
        self.vocabulary = {term: index for index,
                           term in enumerate(self.terms)}
        self.topics = list(set(all_labels))
        self.topic_vocabulary = {
            topic: index for index, topic in enumerate(self.topics)}
        self.K = len(self.topics)
        self.T = len(self.terms)
        self.W = [[self.vocabulary[term] for term in doc_words]
                  for doc_words in doc_corpus]
        self.M = len(self.W)
        ...
        for m in range(self.M):
            ...
            z_vector = [LldaModel._multinomial_sample(
                p_vector) for _ in range(len(self.W[m]))]
    ...
    def _gibbs_sample_training(self):
        ...
        for m in range(self.M):
            ...
            for t, z, n in zip(self.W[m], self.Z[m], range(len(self.W[m]))):
                ...
                numerator_theta_vector = self.Doc2TopicCount[m] + \
                    doc_m_alpha_vector
                ...
                numerator_beta_vector = self.Topic2TermCount[:,
                                                             t] + self.eta_vector[t]
        ...
        for i in range(iteration):
            if log:
                print("after iteration: %s, perplexity: %s" %
                      (self.iteration, self.perplexity()))
    ...
    def inference(self, document, iteration=30, times=10):
        ...
        document = LldaModel._document_preprocess(document)
        doc_words = document.split()
        term_vector = [self.vocabulary[word]
                       for word in doc_words if word in self.vocabulary]
        theta_new = self._gibbs_sample_inference(
            term_vector, iteration=iteration, times=times)
        doc_topic_new = [(self.topics[k], probability)
                         for k, probability in enumerate(theta_new)]
    ...
    def theta_m(self, m):
        ...
        """
        numerator_vector = self.Doc2TopicCount[m] + \
            self.alpha_vector * self.Lambda[m]
    ...
    def log_perplexity(self, documents=None, iteration=30, times=10):
        ...
        if not documents:
            ...
            documents = [LldaModel._document_preprocess(
                document) for document in documents]
            test_corpus = [document.split() for document in documents]
            W = [[self.vocabulary[term] for term in doc_words if term in self.vocabulary]
                 for doc_words in test_corpus]
    ...
    def _write_object_to_file(file_name, target_object):
        ...
        try:
            ...
            message = "Write [%s...] to file [%s] error: json.dump error" % (
                str(target_object)[0:10], file_name)
            print("%s\n\t%s" % (message, e.message))
    ...
    def save_model_to_dir(self, dir_name, save_derivative_properties=False):
        ...
        if save_derivative_properties:
            np.save(os.path.join(dir_name, "Doc2TopicCount.npy"),
                    self.Doc2TopicCount)
            np.save(os.path.join(dir_name, "Topic2TermCount.npy"),
                    self.Topic2TermCount)
            np.save(os.path.join(dir_name, "alpha_vector_Lambda.npy"),
                    self.alpha_vector_Lambda)
            np.save(os.path.join(dir_name, "eta_vector_sum.npy"),
                    self.eta_vector_sum)
            np.save(os.path.join(dir_name, "Topic2TermCountSum.npy"),
                    self.Topic2TermCountSum)
        pass
    ...
    def load_model_from_dir(self, dir_name, load_derivative_properties=True):
        ...
        if load_derivative_properties:
            try:
                self.Doc2TopicCount = np.load(
                    os.path.join(dir_name, "Doc2TopicCount.npy"))
                self.Topic2TermCount = np.load(
                    os.path.join(dir_name, "Topic2TermCount.npy"))
                self.alpha_vector_Lambda = np.load(
                    os.path.join(dir_name, "alpha_vector_Lambda.npy"))
                self.eta_vector_sum = np.load(
                    os.path.join(dir_name, "eta_vector_sum.npy"))
                self.Topic2TermCountSum = np.load(
                    os.path.join(dir_name, "Topic2TermCountSum.npy"))
            except IOError or ValueError as e:
                print(
                    "%s: load derivative properties fail, initialize them with basic properties" % e)
                self._initialize_derivative_fields()
        else:
            self._initialize_derivative_fields()
        pass
    ...
    def update(self, labeled_documents=None):
        ...
        new_w_vectors = [[self.vocabulary[term]
                          for term in doc_words] for doc_words in new_doc_corpus]
    ...
    def top_terms_of_topic(self, topic, k, with_probabilities=True):
        ...
        terms = sorted(list(zip(self.terms, beta)),
